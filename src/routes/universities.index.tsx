...
export const Route = createFileRoute("/universities/")({
  ...
  head: () => ({
    meta: [
      { title: "Browse Ethiopian Universities | ETHIO UNIVERSITY GUIDE" },
      { name: "description", content: "Search and filter Ethiopian universities by region, generation, institution type and field of study." },
      { property: "og:title", content: "Browse Ethiopian Universities | ETHIO UNIVERSITY GUIDE" },
      { property: "og:description", content: "Filter Ethiopian universities by region, generation, type and field of study." },
    ],
  }),
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              <select className={selectClass} value={search.region ?? ""} onChange={(e) => set({ region: e.target.value || undefined })}>
                <option value="">All regions</option>
                {getRegions().map((r) => (<option key={r.id} value={r.id}>{r.name}</option>))}
              </select>
              <select className={selectClass} value={search.generation ?? ""} onChange={(e) => set({ generation: e.target.value || undefined })}>
                <option value="">All generations</option>
                {getGenerations().map((g) => (<option key={g.id} value={g.id}>{g.name}</option>))}
              </select>
              <select className={selectClass} value={search.type ?? ""} onChange={(e) => set({ type: e.target.value || undefined })}>
                <option value="">All types</option>
                {getUniversityTypes().map((t) => (<option key={t} value={t}>{t}</option>))}
              </select>
              <select className={selectClass} value={search.field ?? ""} onChange={(e) => set({ field: e.target.value || undefined })}>
                <option value="">All fields</option>
                {availableProgramFields().map((f) => (<option key={f.key} value={f.key}>{f.label}</option>))}
              </select>
              <select className={selectClass} value={search.sort ?? "name"} onChange={(e) => set({ sort: e.target.value as Filters["sort"] })}>
          ...
          {results.length === 0 ? (
            <div className="mt-4 rounded-xl border border-dashed border-border p-10 text-center">
              <p className="text-sm text-muted-foreground">No matching university found.</p>
            </div>
          ) : (
